...
case 'lofi':
     ...
     case 'sad':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/gMY8Cxu.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cuscuz, kkkkk'})
				     	break
			case 'igstalk':                      if (isLimit(sender)) return reply(ind.limitend(pusname))
...
client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('❌ mande o vídeo dps marque o video com a legenda !tomp3 ❌')
					reply(mess.wait)
